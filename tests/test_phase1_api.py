...
def test_feedback_rejected_for_no_trade_and_wait(client, approved_user):
    from datetime import datetime, timedelta
    from db.session import SessionLocal
    ...
    db = SessionLocal()
    try:
        for action in ("NO_TRADE", "WAIT_FOR_CONFIRMATION"):
            review = PredictionReview(
            ...
            db.refresh(review)
            res = client.post(
                f"/my/reviews/{review.id}/feedback",
                json={"feedback": "SUCCESSFUL", "kind": "outcome"},
                headers=headers,
            )
            assert res.status_code == 400, (action, res.get_json())
    finally:
        db.close()
...
def test_wait_for_confirmation_not_scheduled_for_feedback(client, approved_user):
    from datetime import datetime, timedelta
    from db.session import SessionLocal
    from db.models import PredictionReview
    ...
    try:
        review = PredictionReview(
            ...
            symbol="EURUSD",
            interval="60min",
            predicted_action="WAIT_FOR_CONFIRMATION",
            predicted_confidence=0.5,
            entry_price=1.1,
            predicted_at=datetime.utcnow(),
            evaluate_at=datetime.utcnow() + timedelta(hours=2),
    ...
    headers = {"Authorization": f"Bearer {approved_user['token']}"}
    row = next(
        r for r in client.get("/my/reviews?limit=10", headers=headers).get_json()["reviews"]
        if r["id"] == rid
    )
    assert row["can_record_trade_entry"] is False
    assert row["can_record_outcome"] is False
