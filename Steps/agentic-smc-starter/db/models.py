...
class Account(Base):
    ...
    base_risk_pct = Column(Float, default=0.01)  # default 1% risk per trade
...
class Signal(Base):
    ...
    side = Column(String(4))  # BUY/SELL/SKIP (for SKIP, still useful for stats)
...
class Trade(Base):
    ...
    status = Column(TradeStatus, default='OPEN', index=True)
    ...
    pnl = Column(Float, nullable=True)  # in account currency (USD assumed by default)
